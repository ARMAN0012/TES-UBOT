...
async def _(fry):
    await fry.edit("`MUKA LU UDAH KAYA KONTOL, NI GUA JELEKKIN LAGI🗿`")
    level = fry.pattern_match.group(2)
    if fry.fwd_from:
        return
    if not fry.reply_to_msg_id:
        await fry.edit("`JADI MANUSIA JANGAN BODO,BALES KE STICKER NYA LAH KONTOLLL`")
        return
    ...
    if reply_message.sender.bot:
        await fry.edit("`JADI MANUSIA JANGAN BODO, BALES KE STICKER NYA LAH KONTOLL`")
        return
