...
from userbot.utils import bash, edit_delete, edit_or_reply, kar_cmd
...
async def vc_play(event):
    ...
    if (
        ...
        return await edit_or_reply(event, "**JUDUL LAGUNYA KETIK DULU LH NGENTOD**")
    elif replied and not replied.audio and not replied.voice or not replied:
        karbot = await edit_or_reply(event, "`Sedang mencari lagu haram....`")
        query = event.text.split(maxsplit=1)[1]
        search = ytsearch(query)
        if search == 0:
            await karbot.edit(
                "**Ko Gua ga nemu lagunya ya bang** Coba cari Pake Judul Yg spesifik kontol"
            )
        else:
            songname = search[0]
            title = search[0]
            ...
            if hm == 0:
                ...
                try:
                    ...
                    clear_queue(chat_id)
                    await karbot.edit(f"`{ep}`")

    else:
        karbot = await edit_or_reply(event, "📥 **Sedang Mendownload**")
